...
public class Cql2ExpressionValidatorTests
{
    ...
    {
        var expression = new Cql2FunctionCallExpression("contains", [null!]);

        Assert.Throws<FormatException>(() => Cql2ExpressionValidator.Validate(expression));
    }

    /// <summary>
    /// Verifies that CQL2 numeric literals validate.
    /// </summary>
    [Fact]
    public void Validate_NumericLiteral_DoesNotThrow()
    {
        var expression = new Cql2NumberExpression(1m);
    ...
    {
        var expression = new Cql2FunctionCallExpression("contains", [new Cql2PropertyExpression("name"), new Cql2StringExpression("x")]);
